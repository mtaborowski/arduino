...
#define _DEBUG_TXPIN_ 6
#define _DEBUG_RXPIN_ 7	
SoftwareSerial debug(_DEBUG_TXPIN_, _DEBUG_RXPIN_);
...
int GSM::begin(long baud_rate){
	#ifdef UNO
		if (baud_rate==115200){
			debug.println("Don't use baudrate 115200 with Software Serial.\nAutomatically changed at 9600.");
			baud_rate=9600;
		}
	#endif
	int response=-1;
	...
	*/
	...
	else
	{
	  ...
	  */
		...
		debug.print("ATT: ");
		debug.println(compare_string);
		debug.print("RIC: NO STRING RCVD");
	#endif
  }
